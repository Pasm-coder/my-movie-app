...
@movie_direction_router.get('/movie/{director_id}/direction/', tags=['direction'],response_model=list[MovieDirection],status_code=200)
def get_movie_direction(director_id:int ):
    db = Session()
    result = MovieDirectionService(db).get_movie_direction(director_id)
    if not result:
